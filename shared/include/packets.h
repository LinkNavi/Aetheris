...
enum class PacketID : uint8_t {
  ...
  TreeSpawn = 0x08,
 WaterChunkData = 0x09,
 WaterPlace     = 0x0A,  // client -> server: place water source
 SpawnTree      = 0x0B,  // client -> server: spawn tree at position (debug)
};
...
inline void writeI32(std::vector<uint8_t> &b, int32_t v) {
  writeU32(b, (uint32_t)v);
}
...
inline float readF32(const uint8_t *d, size_t &o) {
  uint32_t tmp = readU32(d, o);
  float v;
  memcpy(&v, &tmp, 4);
  return v;
}
inline int32_t readI32(const uint8_t *d, size_t &o) {
...
struct ChunkDataPacket {
  ...
  static ChunkDataPacket deserialize(const uint8_t *d, size_t len) {
    ...
    p.coord.x = readI32(d, o);
    p.coord.y = readI32(d, o);
    p.coord.z = readI32(d, o);
    uint32_t vc = readU32(d, o);
    p.vertices.resize(vc);
    for (auto &f : p.vertices)
      f = readF32(d, o);
    uint32_t mc = readU32(d, o);
    p.materials.resize(mc);
    for (auto &mat : p.materials)
      mat = readU32(d, o);
...
struct TreeSpawnPacket {
  ...
  std::vector<uint8_t> serialize() const {
    std::vector<uint8_t> b;
    writeU8(b, (uint8_t)PacketID::TreeSpawn);
...
struct RenderDistPacket {
  ...
  std::vector<uint8_t> serialize() const {
    std::vector<uint8_t> b;
    writeU8(b, (uint8_t)PacketID::RenderDist);
    writeU8(b, xz);
    writeU8(b, y);
    return b;
  }
...
// ── WaterChunkData packet (server -> client) ──────────────────────────────────
// Sent when a water chunk changes. Contains the full water level grid.
 
struct WaterChunkDataPacket {
    ChunkCoord coord;
    // Flat array: SIZE*SIZE*SIZE bytes, one per voxel
    std::array<uint8_t, ChunkData::SIZE * ChunkData::SIZE * ChunkData::SIZE> levels{};
 
    std::vector<uint8_t> serialize() const {
        std::vector<uint8_t> b;
        writeU8(b, (uint8_t)PacketID::WaterChunkData);
        writeI32(b, coord.x); writeI32(b, coord.y); writeI32(b, coord.z);
        b.insert(b.end(), levels.begin(), levels.end());
        return b;
    }
    ...
    static WaterChunkDataPacket deserialize(const uint8_t* d, size_t) {
        ...
        for (auto& v : p.levels) v = readU8(d, o);
        return p;
    }
};
 
// ── WaterPlacePacket (client -> server) ───────────────────────────────────────
struct WaterPlacePacket {
    int32_t wx, wy, wz;
    uint8_t level; // 8 = source, 0 = remove
 
    std::vector<uint8_t> serialize() const {
        std::vector<uint8_t> b;
        writeU8(b, (uint8_t)PacketID::WaterPlace);
        writeI32(b, wx); writeI32(b, wy); writeI32(b, wz);
        writeU8(b, level);
        return b;
    }
...
// ── SpawnTreePacket (client -> server, debug only) ────────────────────────────
struct SpawnTreePacket {
    float wx, wy, wz;
 
    std::vector<uint8_t> serialize() const {
        std::vector<uint8_t> b;
        writeU8(b, (uint8_t)PacketID::SpawnTree);
