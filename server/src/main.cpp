    ...
    while (true) {
        ...
        while (enet_host_service(host.get(), &ev, 0) > 0) {
            ...
            case ENET_EVENT_TYPE_CONNECT: {
                ...
                // Schedule initial chunks async — they'll arrive via flushReady
                // SpawnPosition is sent after so the client gate waits for chunks
                chunks.updateClient(ev.peer, 0.f, spawnY, 0.f);

                // We still need to send SpawnPosition; do it now so the client
                // knows where to stand while chunks stream in.
