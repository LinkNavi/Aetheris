...
int main(int argc, char **argv) {
    ...
    spellMgr.registerNative("aoe_damage", [&](Aether::NativeArgs args,
        ...
        if (args.empty() || !args[0].isVec3()) return Aether::Value::null();
        auto   pos    = args[0].asVec3();
        float  radius = named.count("radius") ? (float)named["radius"].asNumber() : 1.f;
        float  damage = named.count("damage") ? (float)named["damage"].asNumber() : 0.f;
        // TODO: hook into CombatSystem spatial query
        (void)pos; (void)radius; (void)damage;
        return Aether::Value::null();
    });

    spellMgr.registerNative("aoe_heal", [&](Aether::NativeArgs args,
                                             Aether::NativeNamedArgs named) -> Aether::Value {
        if (args.empty() || !args[0].isVec3()) return Aether::Value::null();
        float amount = named.count("amount") ? (float)named["amount"].asNumber() : 0.f;
        // TODO: find players in radius and call statsMgr.applyHeal()
        (void)amount;
        return Aether::Value::null();
    });

    spellMgr.registerNative("apply_status", [&](Aether::NativeArgs args,
                                                  Aether::NativeNamedArgs named) -> Aether::Value {
        // args[0]=entity, args[1]=status string, args[2]=duration
        return Aether::Value::null();
    });

    spellMgr.registerNative("projectile", [&](Aether::NativeArgs args,
                                               Aether::NativeNamedArgs named) -> Aether::Value {
        // args[0]=origin vec3, args[1]=dir vec3
        // named["speed"], named["radius"]
        return Aether::Value::null();
    });

    spellMgr.registerNative("__log__", [](Aether::NativeArgs args,
                                           Aether::NativeNamedArgs) -> Aether::Value {
    ...
    Log::info("Default spawn: " + std::to_string(defaultSpawn.x) + ", " +
              std::to_string(defaultSpawn.y) + ", " + std::to_string(defaultSpawn.z));
    ...
    while (true) {
            ...
            case ENET_EVENT_TYPE_RECEIVE: {
                ...
                if (pid == (uint8_t)MPPacketID::AuthRequest) {
                    ...
                    if (mpMgr.isAuthenticated(ev.peer)) {
                        ...
                        Net::sendReliable(ev.peer, SpawnPositionPacket{sp.x, sp.y, sp.z}.serialize());
                ...
                // ── Spell charge — aim update ─────────────────────────────────
                    ...
                    if (cs) { cs->aimX = pkt.aimX; cs->aimY = pkt.aimY; cs->aimZ = pkt.aimZ; }
                ...
                // ── Chat ──────────────────────────────────────────────────────
                    ...
                    chatMgr.onMessage(ev.peer, ChatMessagePacket::deserialize(d, len), host.get());
                    enet_host_flush(host.get());
                }
        ...
        for (auto& ev2 : firedSpells) {
            ...
            if (len > 0.001f) dir = (targetPos - casterPos) / len;
            ack.dirX = dir.x; ack.dirY = dir.y; ack.dirZ = dir.z;
            ack.hasProjectile = 1;
