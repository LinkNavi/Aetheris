...
CastState& SpellManager::stateFor(ENetPeer* peer) {
    return _castStates[peer];
}
...
bool SpellManager::hasStaffEquipped(const Inventory& inv) const {
    ...
    if (!wpn.empty()) {
        ...
        if (def.type == ItemType::Weapon) return true; // staff check
    }
    // also accept offhand magic item
    const ItemStack& off = inv.offhandSlot();
    if (!off.empty() && getItemDef(off.id).type == ItemType::Magic) return true;
    return false;
}
...
void SpellManager::loadSpellsFromDir(const std::string& dir) {
    ...
    if (!fs::exists(dir)) { Log::warn("Spell dir not found: " + dir); return; }
    for (auto& entry : fs::directory_iterator(dir)) {
        if (entry.path().extension() != ".aes") continue;
...
bool SpellManager::loadSpellSource(const std::string& name, const std::string& src) {
    SpellDef def;
    def.name = name;
    ...
    while (std::getline(lines, line)) {
        ...
        auto parseFloat = [&](const std::string& key, float& out) {
            if (l.rfind(key, 0) == 0) {
                try { out = std::stof(l.substr(key.size())); } catch (...) {}
            }
        };
...
void SpellManager::registerNative(const std::string& name, Aether::NativeFn fn) {
...
bool SpellManager::beginCharge(ENetPeer* peer, const std::string& spellName,
    ...
    if (!def) { Log::warn("Unknown spell: " + spellName); return false; }
    if (stats.dead || stats.mana < def->baseMana) return false;
    if (def->requireStaff && !hasStaffEquipped(inv)) return false;
    if (isOnCooldown(peer, spellName)) return false;

    CastState& cs = stateFor(peer);
    if (!cs.isIdle()) return false; // already casting
    ...
    cs.aimX = aimX; cs.aimY = aimY; cs.aimZ = aimZ;
...
void SpellManager::tickCharge(ENetPeer* peer, float dt, PlayerStats& stats) {
    CastState& cs = stateFor(peer);
    if (!cs.isCharging()) return;

    cs.manaCommitted += cs.chargeRate * dt;
    cs.manaCommitted  = std::min(cs.manaCommitted, cs.maxCharge);
    cs.manaCommitted  = std::min(cs.manaCommitted, stats.mana);

    // Drain mana in real time so the player feels the commitment
    stats.mana -= cs.chargeRate * dt;
    stats.mana  = std::max(stats.mana, 0.f);
    stats.clamp();

    // Auto-commit if mana hits 0 or cap
...
bool SpellManager::commitCast(ENetPeer* peer, PlayerStats& stats) {
    CastState& cs = stateFor(peer);
    if (!cs.isCharging()) return false;

    SpellDef* def = findSpell(cs.spellName);
    if (!def) { cs.reset(); return false; }

    float mana = cs.manaCommitted;
    if (mana < def->baseMana) {
        // Refund — not enough mana committed
        stats.mana += mana;
        stats.clamp();
        cs.reset();
        return false;
    ...
    cs.castTimeTotal   = CastState::calcCastTime(mana, def->baseTime, def->timeScale);
    cs.castTimeElapsed = 0.f;
    cs.interruptDC     = CastState::calcInterruptDC(mana);
...
void SpellManager::cancelCast(ENetPeer* peer) {
    ...
    if (cs.isCharging()) {
        ...
        // (caller is responsible for refunding via onDamageTaken or explicit cancel)
    }
...
bool SpellManager::onDamageTaken(ENetPeer* peer, float damage, PlayerStats& stats) {
    CastState& cs = stateFor(peer);
    if (!cs.isWindingUp()) return false;
...
void SpellManager::applyCooldown(ENetPeer* peer, const std::string& spell, float mana) {
    _cooldowns[peer][spell] = calcCooldown(mana);
}

bool SpellManager::isOnCooldown(ENetPeer* peer, const std::string& spell) const {
    auto pit = _cooldowns.find(peer);
    if (pit == _cooldowns.end()) return false;
    auto sit = pit->second.find(spell);
    if (sit == pit->second.end()) return false;
    return sit->second > 0.f;
}

float SpellManager::getCooldown(ENetPeer* peer, const std::string& spell) const {
    auto pit = _cooldowns.find(peer);
    if (pit == _cooldowns.end()) return 0.f;
    auto sit = pit->second.find(spell);
    if (sit == pit->second.end()) return 0.f;
