...
public:
  ...
  void savePlayerPos(uint64_t uid, glm::vec3 pos) {
    std::string path =
        std::string(PLAYER_INV_DIR) + std::to_string(uid) + ".pos";
  ...
  const Inventory *getPlayerInvPublic(ENetPeer *peer) const {
  ...
  void sendInventoryState(ENetPeer *peer) {
    auto it = _playerInvs.find(peer);
    if (it == _playerInvs.end())
      return;
    InventoryStatePacket p;
    p.inv = it->second;
  ...
  void onChestOpenReq(ENetPeer *peer, const ChestOpenReqPacket &req) {
    ...
    if (pit == _playerPos.end() || cit == _chests.end())
      return;

    ServerChest &c = cit->second;
    if (glm::length(pit->second - c.pos) > CHEST_INTERACT_RANGE)
      return;
    if (c.lockedBy && c.lockedBy != peer)
      return;
  ...
  void onInventoryMoveReq(ENetPeer *peer, const InventoryMoveReqPacket &req) {
    ...
    if (!srcInv || !dstInv) {
      Log::warn("MoveReq: invalid inventory");
      return;
    }
    ...
    if (!srcSlot || !dstSlot) {
      Log::warn("MoveReq: invalid slot");
      return;
    }

    if (!validateMove(*srcSlot, req.dst) || !validateMove(*dstSlot, req.src)) {
      Log::warn("MoveReq: type guard failed");
      return;
    }

    // Stack merge for same item in grid/hotbar
    if (req.src.region != SlotRegion::Equip &&
        req.dst.region != SlotRegion::Equip && !srcSlot->empty() &&
        !dstSlot->empty() && srcSlot->id == dstSlot->id) {
    ...
    if (req.src.owner == InvOwner::Chest || req.dst.owner == InvOwner::Chest) {
      ...
      if (it != _chests.end()) {
        secInv = &it->second.inv;
        saveChests();
      }
    } else if (req.src.owner == InvOwner::Corpse ||
               req.dst.owner == InvOwner::Corpse) {
...
private:
  ...
  Inventory *resolveInv(ENetPeer *peer, const SlotRef &ref) {
    switch (ref.owner) {
    case InvOwner::Player:
      return getPlayerInv(peer);
    case InvOwner::Chest: {
    ...
    case InvOwner::Corpse: {
      ...
      if (it == _corpses.end())
        return nullptr;
      if (it->second.killer && it->second.killer != peer)
        return nullptr;
      return &it->second.inv;
    }
    }
  ...
  bool validateMove(const ItemStack &stack, const SlotRef &dst) {
    if (dst.region != SlotRegion::Equip)
      return true; // grid/hotbar accept anything
    if (stack.empty())
      return true;
    const ItemDef &def = getItemDef(stack.id);
    if (dst.index == EQUIP_WEAPON)
      return def.type == ItemType::Weapon;
    if (dst.index == EQUIP_OFFHAND)
      return def.type == ItemType::Shield || def.type == ItemType::Magic;
    if (dst.index >= EQUIP_TOTEM_0 && dst.index < EQUIP_TOTEM_0 + TOTEM_SLOTS)
  ...
  void writeInv(std::ofstream &f, const Inventory &inv) {
    ...
    for (const auto &s : inv.grid)
      writeSlot(s);
    for (const auto &s : inv.equipSlots)
      writeSlot(s);
    for (const auto &s : inv.hotbars)
      writeSlot(s);
  }
  ...
  void readInv(std::ifstream &f, Inventory &inv) {
    ...
    for (auto &s : inv.grid)
      readSlot(s);
    for (auto &s : inv.equipSlots)
      readSlot(s);
    for (auto &s : inv.hotbars)
      readSlot(s);
  }
  ...
  void savePlayerInv(uint64_t uid, const Inventory &inv) {
    std::string path =
        std::string(PLAYER_INV_DIR) + std::to_string(uid) + ".inv";
    std::ofstream f(path, std::ios::binary | std::ios::trunc);
    if (f)
      writeInv(f, inv);
  }
