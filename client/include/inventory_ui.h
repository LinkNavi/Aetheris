...
// ── InventoryUI
// ─────────────────────────────────────────────────────────────── Hotbar is
// always visible (drawn even when inventory is closed). Tab cycles Combat →
// Items → Blocks. 1-8 selects active hotbar slot. All mutations go to server
// via InventoryMoveReqPacket.
class InventoryUI {
public:
  ...
  bool draw(CInventory &cinv, ClientChestMirror *chest, ENetPeer *server) {
    ...
    const ItemStack &mainhand = cinv.inv.weaponSlot();
    if (offhand.id == ItemID::WpnGrimoire || mainhand.id == ItemID::WpnStaff) {
      drawSpellSlots(cinv);
    }
    if (cinv.open) {
  ...
  // Returns true if a hotbar slot changed (caller may want to update
  // viewmodel).
...
private:
  ...
  void syncWeaponFromHotbar(CInventory &cinv) {
    if (cinv.hotbarMode != HotbarMode::Combat)
      return;
    const ItemStack &active =
        cinv.inv.hotbarSlot(HotbarMode::Combat, cinv.hotbarActive);
  ...
  void drawHotbar(CInventory &cinv) {
    ...
    if (ImGui::Button(HOTBAR_MODE_NAMES[(int)cinv.hotbarMode],
                      {90.f, SLOT_SZ})) {
      int next = ((int)cinv.hotbarMode + 1) % HOTBAR_MODE_COUNT;
      cinv.hotbarMode = (HotbarMode)next;
      syncWeaponFromHotbar(cinv);
    ...
    for (int i = 0; i < HOTBAR_SIZE; i++) {
      ...
      if (active) {
        ImGui::PushStyleColor(ImGuiCol_Button, {0.55f, 0.75f, 0.25f, 1.f});
      }
  ...
  void drawSpellSlots(CInventory &cinv) {
    ImGuiIO &io = ImGui::GetIO();

    float hotbarW = HOTBAR_SIZE * (SLOT_SZ + SLOT_PAD) + SLOT_PAD + 120.f;
    float spellW = SPELL_SLOTS * (SLOT_SZ + SLOT_PAD) + SLOT_PAD;
    float barH = SLOT_SZ + 36.f;

    // Position to the right of the hotbar
    float hotbarX = (io.DisplaySize.x - hotbarW) * 0.5f;
    float x = hotbarX + hotbarW + 8.f;
    float y = io.DisplaySize.y - barH - 8.f;

    ImGui::SetNextWindowPos({x, y}, ImGuiCond_Always);
    ImGui::SetNextWindowSize({spellW, barH}, ImGuiCond_Always);
    ImGui::SetNextWindowBgAlpha(0.75f);
    ImGui::Begin("##spell-slots", nullptr,
                 ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav |
                     ImGuiWindowFlags_NoMove |
                     ImGuiWindowFlags_NoSavedSettings |
                     ImGuiWindowFlags_NoBringToFrontOnFocus);
    ...
    for (int i = 0; i < SPELL_SLOTS; i++) {
      ...
      bool active = (i == cinv.activeSpellSlot);
      int bookIdx = (i < SPELL_SLOTS) ? cinv.inv.activeSpells[i] : -1;
      bool hasSpell = (bookIdx >= 0 && bookIdx < SPELL_BOOK_SIZE &&
                       !cinv.inv.spellBook[bookIdx].empty());

      if (active)
        ImGui::PushStyleColor(ImGuiCol_Button, {0.20f, 0.10f, 0.55f, 1.f});

      // Draw the slot — use empty ItemStack, spell name as key hint
      ItemStack dummy{};
      const char *label =
          hasSpell ? cinv.inv.spellBook[bookIdx].displayName.c_str() : nullptr;
      drawSlot(dummy, InvOwner::Player, 0, SlotRegion::Grid, (uint8_t)(50 + i),
               label);

      // Draw spell name abbreviation manually if has spell
      if (hasSpell) {
        ImVec2 p = ImGui::GetItemRectMin();
        ImDrawList *dl = ImGui::GetWindowDrawList();
        const std::string &dn = cinv.inv.spellBook[bookIdx].displayName;
        char abbr[4] = {};
        for (int c = 0; c < 3 && dn[c]; c++)
          abbr[c] = dn[c];
        dl->AddText({p.x + 3, p.y + 3}, IM_COL32(180, 120, 255, 255), abbr);
      }
  ...
  void drawInventoryWindow(CInventory &cinv, ClientChestMirror *chest) {
    ...
    ImGui::Text("Weapon ");
    ImGui::SameLine();
    drawSlot(cinv.inv.weaponSlot(), InvOwner::Player, 0, SlotRegion::Equip,
             EQUIP_WEAPON);
    ...
    ImGui::Text("Offhand");
    ImGui::SameLine();
    drawSlot(cinv.inv.offhandSlot(), InvOwner::Player, 0, SlotRegion::Equip,
             EQUIP_OFFHAND);
    ...
    for (int t = 0; t < TOTEM_SLOTS; t++) {
      ImGui::SameLine(0, SLOT_PAD);
      drawSlot(cinv.inv.totemSlot(t), InvOwner::Player, 0, SlotRegion::Equip,
    ...
    for (int m = 0; m < HOTBAR_MODE_COUNT; m++) {
      ...
      for (int i = 0; i < HOTBAR_SIZE; i++) {
        ...
        drawSlot(cinv.inv.hotbarSlot(mode, i), InvOwner::Player, 0,
                 SlotRegion::Hotbar, flat);
  ...
  void drawSlot(const ItemStack &slot, InvOwner owner, uint32_t uid,
    ...
    ImGui::PushID((int)owner * 1000000 + (int)uid * 10000 + (int)region * 1000 +
                  index);
    ...
    if (!empty) {
      ...
      const ItemDef &def = getItemDef(slot.id);
      char abbr[4] = {};
      for (int i = 0; i < 3 && def.name[i]; i++)
        abbr[i] = def.name[i];
      dl->AddText({p.x + 3, p.y + 3}, IM_COL32(230, 230, 230, 255), abbr);
      if (def.maxStack > 1) {
        char cnt[8];
        snprintf(cnt, sizeof(cnt), "%d", slot.count);
        dl->AddText({p.x + 3, p.y + SLOT_SZ - 14}, IM_COL32(255, 220, 80, 255),
                    cnt);
      }
    }
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *p = ImGui::AcceptDragDropPayload(DRAG_TYPE)) {
        const DragPayload &src = *static_cast<const DragPayload *>(p->Data);
  ...
  ImVec4 modeColor(HotbarMode mode) {
    switch (mode) {
    case HotbarMode::Combat:
      return {0.55f, 0.15f, 0.15f, 0.90f};
    case HotbarMode::Items:
      return {0.15f, 0.45f, 0.55f, 0.90f};
    case HotbarMode::Blocks:
      return {0.40f, 0.30f, 0.15f, 0.90f};
    default:
      return {0.25f, 0.25f, 0.25f, 0.90f};
    }
  }
};
