...
static inline ImU32 tokenColor(Aether::TokenType t) {
    ...
    switch (t) {
    case T::KwSpell: case T::KwRune: case T::KwFn:
    case T::KwReturn: case T::KwIf: case T::KwElse:
    case T::KwFor: case T::KwWhile: case T::KwBreak:
    case T::KwContinue: case T::KwLet: case T::KwVec3:
    case T::KwFail: case T::KwLog:
        return IM_COL32(200, 120, 255, 255); // purple — keywords

    case T::KwTrue: case T::KwFalse: case T::KwNull:
    ...
    case T::Percent: case T::Eq: case T::EqEq: case T::BangEq:
    case T::Lt: case T::Gt: case T::LtEq: case T::GtEq:
    case T::And: case T::Or: case T::Bang:
    case T::PlusEq: case T::MinusEq: case T::StarEq: case T::SlashEq:
        return IM_COL32(255, 200, 100, 255); // yellow — operators

    case T::LParen: case T::RParen: case T::LBrace:
    case T::RBrace: case T::LBracket: case T::RBracket:
        return IM_COL32(200, 200, 200, 255); // white — brackets

    case T::Colon: case T::Comma: case T::Dot: case T::Semicolon:
...
private:
    ...
    void drawSpellList(CInventory& cinv) {
        ...
        for (int i = 0; i < SPELL_BOOK_SIZE; i++) {
            ...
            // Active indicator
            ImU32 dotCol = isActive ? IM_COL32(80,255,120,255) : IM_COL32(80,80,80,200);
            ImDrawList* dl = ImGui::GetWindowDrawList();
            ImVec2 p = ImGui::GetCursorScreenPos();
            dl->AddCircleFilled({p.x+8.f, p.y+10.f}, 5.f, dotCol, 8);
            ImGui::Dummy({0.f, 0.f}); ImGui::SameLine(18.f);
    ...
    void drawEditor(CInventory& cinv, ENetPeer* server, float w, float h) {
        ...
        // Invisible InputTextMultiline captures keyboard input
        ImGui::PushStyleColor(ImGuiCol_FrameBg, {0.f,0.f,0.f,0.f});
        ImGui::PushStyleColor(ImGuiCol_Text, {0.f,0.f,0.f,0.f}); // invisible text
        ...
        if (changed) {
            _debounceTimer = 0.4f; // re-highlight after 400ms idle
        }
        ...
        if (_selectedSpell >= 0 && _selectedSpell < SPELL_BOOK_SIZE &&
            ...
            for (int s = 0; s < SPELL_SLOTS; s++) {
                ...
                if (ImGui::Button(btnLabel, {-1.f, 26.f})) {
                    ...
                    // Send loadout update
                    if (server) sendLoadout(cinv, server);
                }
    ...
    void _rebuildHighlight() {
        ...
        for (auto& tok : tokens) {
            if (tok.type == Aether::TokenType::Eof) break;
            int lineIdx = tok.line - 1; // 1-based → 0-based
            if (lineIdx < 0 || lineIdx >= (int)_hlLines.size()) continue;
    ...
    void _rebuildErrorLines() {
        ...
        while ((p = e.find("line ", p)) != std::string::npos) {
            ...
            if (lineNo > 0) _errorLines.push_back(lineNo);
        }
    }

    void drawHighlight(ImVec2 editorPos, float w, float h) {
        if (_hlLines.empty()) return;
        ...
        float scrollY = win ? win->Scroll.y : 0.f;
        ...
        for (int li = firstLine; li <= lastLine; li++) {
            ...
            for (const HlSpan& sp : hl.spans) {
                ...
                for (int row = 0; row < li && *buf; buf++) {
                    if (*buf == '\n') { row++; lineStart = (int)(buf - _editorBuf + 1); }
                }
            ...
            if (hl.spans.empty()) {
                ...
                if (lineLen > 0)
                    dl->AddText(font, fontSize, {editorPos.x + 4.f, ly},
                                IM_COL32(210,210,210,255), linePtr,
                                linePtr + lineLen);
            }
        }
        dl->PopClipRect();
    ...
    void drawErrorUnderlines(ImVec2 editorPos, float w) {
        ...
        for (int errLine : _errorLines) {
            ...
            dl->AddLine({editorPos.x + 4.f, ly},
                        {editorPos.x + w - 4.f, ly},
