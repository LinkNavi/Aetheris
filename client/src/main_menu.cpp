...
static constexpr ImU32 COL_TEAL_DIM =
    IM_COL32(20, 70, 110, 100); // dim light blue
...
void MainMenu::drawParticles(ImDrawList *dl, int sw, int sh) {
  for (int i = 0; i < MAX_PARTICLES; i++) {
    ...
    float alpha = p.life * p.brightness *
                  (0.5f + 0.5f * std::sin(_time * 6.f + i * 1.3f));
    float gx = p.x * sw, gy = p.y * sh;
    dl->AddCircleFilled({gx, gy}, p.size * 2.f,
                        colAlpha(COL_GREY_GLOW, alpha * 0.10f));
...
static void drawBackground(ImDrawList *dl, float time, int sw, int sh) {
  // Pure black gradient
  dl->AddRectFilledMultiColor({0, 0}, {(float)sw, (float)sh}, COL_BG_DARK,
                              COL_BG_DARK, COL_BG_MID, COL_BG_MID);
  ...
  for (int y = 0; y < sh; y += 4)
    dl->AddLine({0.f, (float)y}, {(float)sw, (float)y}, IM_COL32(0, 0, 0, 8),
                1.f);

  // Corner ornaments in light blue
  float orn = 50.f;
  ImU32 ornCol = colAlpha(COL_TEAL_DIM, 0.6f);
  dl->AddLine({0, 0}, {orn, 0}, ornCol, 1.f);
  dl->AddLine({0, 0}, {0, orn}, ornCol, 1.f);
  ...
  dl->AddLine({(float)sw, (float)sh}, {(float)sw - orn, (float)sh}, ornCol,
              1.f);
  dl->AddLine({(float)sw, (float)sh}, {(float)sw, (float)sh - orn}, ornCol,
              1.f);
}
...
// ── Button
// ────────────────────────────────────────────────────────────────────
bool MainMenu::menuButton(ImDrawList *dl, const char *label, float cx, float y,
                          float w, float h, bool /*unused*/, bool &outHovered) {
  float x = cx - w * 0.5f;
  ...
  dl->AddText(font, fsz, {tx2, ty2}, colLerp(COL_WHITE_DIM, COL_WHITE, hov),
              label);
  return clicked;
}
...
static void drawPanel(ImDrawList *dl, float x, float y, float w, float h,
  ...
  dl->AddRectFilled({x + ox, y}, {x + w + ox, y + h},
                    colAlpha(COL_PANEL, alpha), 5.f);
  // Light blue border, subtle
  dl->AddRect({x + ox, y}, {x + w + ox, y + h},
              colAlpha(COL_PANEL_EDGE, alpha * 0.7f), 5.f, 0, 1.f);
  // Top highlight line — light blue
  dl->AddLine({x + ox + 12, y + 1}, {x + w + ox - 12, y + 1},
              colAlpha(COL_TEAL_DIM, alpha * 0.5f), 1.f);
}

void MainMenu::drawSlider(ImDrawList *dl, const char *label, float x, float y,
                          float w, float &value, float mn, float mx,
                          const char *fmt) {
  ImFont *font = ImGui::GetFont();
  ...
  dl->AddCircleFilled({kx, trackY}, knobR,
                      hov ? COL_TEAL : colAlpha(COL_TEAL, 0.5f), 12);
...
void MainMenu::drawToggle(ImDrawList *dl, const char *label, float x, float y,
                          bool &value) {
  ImFont *font = ImGui::GetFont();
  ...
  dl->AddRect({tx, y}, {tx + tw, y + th}, colAlpha(COL_PANEL_EDGE, 0.8f), tr, 0,
              1.f);
  float kx = value ? tx + tw - tr : tx + tr;
  dl->AddCircleFilled({kx, y + tr}, tr - 3.f, value ? COL_TEAL : COL_WHITE_DIM,
                      12);
  ImGui::SetCursorScreenPos({tx, y});
  ImGui::PushID(label);
  if (ImGui::InvisibleButton("##tog", {tw, th}))
    value = !value;
  ImGui::PopID();
}

void MainMenu::drawInputInt(ImDrawList *dl, const char *label, float x, float y,
                            float w, int &value, int mn, int mx) {
  ImFont *font = ImGui::GetFont();
  float labelW = 150.f, fieldW = 60.f, btnSize = 20.f, fx = x + labelW;
  dl->AddText(font, 15.f, {x, y + 2}, COL_WHITE_DIM, label);
  ImGui::PushID(label);
  dl->AddRectFilled({fx, y}, {fx + btnSize, y + btnSize},
                    IM_COL32(12, 18, 26, 220), 2.f);
  dl->AddRect({fx, y}, {fx + btnSize, y + btnSize},
              colAlpha(COL_PANEL_EDGE, 0.6f), 2.f, 0, 1.f);
  ...
  dl->AddRectFilled({vx, y}, {vx + fieldW, y + btnSize},
                    IM_COL32(8, 12, 18, 240), 2.f);
  dl->AddRect({vx, y}, {vx + fieldW, y + btnSize},
              colAlpha(COL_PANEL_EDGE, 0.6f), 2.f, 0, 1.f);
  char buf[16];
  snprintf(buf, sizeof(buf), "%d", value);
  ImVec2 vsz = font->CalcTextSizeA(15.f, 999.f, 0.f, buf);
  dl->AddText(font, 15.f, {vx + (fieldW - vsz.x) * 0.5f, y + 2}, COL_WHITE,
              buf);
  float bx = vx + fieldW + 4;
  dl->AddRectFilled({bx, y}, {bx + btnSize, y + btnSize},
                    IM_COL32(12, 18, 26, 220), 2.f);
  dl->AddRect({bx, y}, {bx + btnSize, y + btnSize},
              colAlpha(COL_PANEL_EDGE, 0.6f), 2.f, 0, 1.f);
...
void MainMenu::drawTextInput(ImDrawList *dl, const char *label, float x,
                             float y, float fieldW, char *buf, int bufSz,
                             bool password) {
  ImFont *font = ImGui::GetFont();
  float labelW = 150.f, fx = x + labelW;
  dl->AddText(font, 15.f, {x, y + 2}, COL_WHITE_DIM, label);
  dl->AddRectFilled({fx, y}, {fx + fieldW, y + 22}, IM_COL32(8, 12, 18, 240),
                    2.f);
  dl->AddRect({fx, y}, {fx + fieldW, y + 22}, colAlpha(COL_PANEL_EDGE, 0.7f),
              2.f, 0, 1.f);
...
static void drawSectionHeader(ImDrawList *dl, ImFont *font, const char *text,
  ...
  dl->AddText(font, 13.f, {x + (w - tsz.x) * 0.5f, y}, colAlpha(COL_TEAL, 0.6f),
              text);
  dl->AddLine({x + (w + tsz.x) * 0.5f + 8, y + 6}, {x + w, y + 6},
              colAlpha(COL_TEAL_DIM, 0.8f), 1.f);
}

static bool drawTab(ImDrawList *dl, ImFont *font, const char *label, float x,
                    float y, float w, float h, bool active) {
  ImU32 bg = active ? IM_COL32(10, 20, 35, 240) : IM_COL32(6, 10, 16, 180);
  dl->AddRectFilled({x, y}, {x + w, y + h}, bg, 3.f);
  if (active)
    dl->AddLine({x + 2, y}, {x + w - 2, y}, COL_TEAL,
                2.f); // light blue top bar
  else
    dl->AddLine({x, y + h}, {x + w, y + h}, colAlpha(COL_PANEL_EDGE, 0.4f),
                1.f);
...
// ── Account Button
// ────────────────────────────────────────────────────────────
void MainMenu::drawAccountButton(ImDrawList *dl, int sw, int sh,
                                 GameState &next) {
  ImFont *font = ImGui::GetFont();
...
// ── Main Page
// ─────────────────────────────────────────────────────────────────
GameState MainMenu::drawMainPage(ImDrawList *dl, float cx, float cy, int sw,
                                 int sh, float dt) {
...
// ── Account Page
// ──────────────────────────────────────────────────────────────
GameState MainMenu::drawAccount(ImDrawList *dl, float cx, float cy, int sw,
                                int sh, float dt) {
  ImFont *font = ImGui::GetFont();
  ...
  if (_account.loggedIn) {
    ...
    dl->AddText(font, 26.f, {cx - hsz.x * 0.5f, panY + 14.f}, COL_WHITE,
                "ACCOUNT");
    ...
    drawSectionHeader(dl, font, "SESSION", lx, cy2, panW - 60.f);
    cy2 += 22.f;
    ...
    dl->AddText(font, 12.f, {lx + 50, cy2 + 2}, colAlpha(COL_TEAL, 0.8f),
                tokenPreview);
    cy2 += rowH;

    float bbY = panY + panH - 54.f;
    bool hBack = false, hLogout = false;
    if (menuButton(dl, "BACK", panX + 80.f + 50.f, bbY, 100.f, 36.f, false,
                   hBack)) {
      _panelSlide = 1.f;
      return GameState::MainMenu;
    }
    if (menuButton(dl, "SIGN OUT", panX + panW - 160.f + 80.f, bbY, 140.f, 36.f,
                   false, hLogout)) {
  ...
  drawTextInput(dl, "Username", lx, cy2, 200.f, _accUser, sizeof(_accUser));
  cy2 += rowH;
  drawTextInput(dl, "Password", lx, cy2, 200.f, _accPass, sizeof(_accPass),
                true);
  cy2 += rowH;
  if (!_accLoginMode) {
    drawTextInput(dl, "Confirm Pass", lx, cy2, 200.f, _accPass2,
                  sizeof(_accPass2), true);
    cy2 += rowH;
  }
  cy2 += 8.f;
  ...
  if (menuButton(dl, "BACK", panX + 60.f + 50.f, bbY, 100.f, 36.f, false,
                 hBack)) {
    _panelSlide = 1.f;
    return GameState::MainMenu;
  }
  const char *actionLbl = _accLoginMode ? "SIGN IN" : "REGISTER";
  if (menuButton(dl, actionLbl, panX + panW - 160.f + 80.f, bbY, 140.f, 36.f,
                 false, hAction)) {
    if (_accUser[0] == 0) {
    ...
    std::string json = std::string("{\"username\":\"") + _accUser +
                       "\",\"password\":\"" + _accPass + "\"}";
    auto resp =
        HttpClient::post("127.0.0.1", _settings.authPort, endpoint, json);
    if (resp.ok()) {
      auto extract = [&](const std::string &body,
      ...
      if (!parsed.empty())
        errMsg = parsed;
      if (resp.status == 0)
        errMsg = "Cannot reach auth server.";
...
// ── Multiplayer
// ───────────────────────────────────────────────────────────────
GameState MainMenu::drawMultiplayer(ImDrawList *dl, float cx, float cy, int sw,
                                    int sh, float dt) {
  ...
  if (_account.loggedIn) {
    ...
    dl->AddText(font, 14.f, {lx + 16, cy2 + 2}, COL_AMBER,
                "Not signed in — connect as guest");
  }
  cy2 += rowH;

  drawSectionHeader(dl, font, "SERVER", lx, cy2, panW - 60.f);
  cy2 += 22.f;
  drawTextInput(dl, "Server IP", lx, cy2, 200.f, _serverInput,
                sizeof(_serverInput));
  cy2 += rowH;
  drawInputInt(dl, "Port", lx, cy2, panW - 60.f, _portInput, 1024, 65535);
  cy2 += rowH + 8.f;
  ...
  if (menuButton(dl, "CONNECT", panX + panW - 160.f + 80.f, bbY, 140.f, 36.f,
                 false, hConn)) {
    snprintf(pendingUsername, sizeof(pendingUsername), "%s",
             _account.loggedIn ? _account.username : "Guest");
    snprintf(_settings.lastServer, sizeof(_settings.lastServer), "%s",
             _serverInput);
...
// ── Settings
// ──────────────────────────────────────────────────────────────────
GameState MainMenu::drawSettings(ImDrawList *dl, float cx, float cy, int sw,
                                 int sh, float dt) {
  ...
  dl->AddText(font, 28.f, {cx - hsz.x * 0.5f, panY + 14.f}, COL_WHITE,
              "SETTINGS");
  dl->AddLine({panX + 20, panY + 50.f}, {panX + panW - 20, panY + 50.f},
              colAlpha(COL_TEAL_DIM, 0.8f), 1.f);
  ...
  if (_settingsTab == 0) {
    ...
    drawSlider(dl, "Render Distance", lx, cy2, panW - 60.f,
               _settings.renderDistance, 1.f, 128.f, "%.0f chunks");
    cy2 += rowH;
    drawSlider(dl, "Field of View", lx, cy2, panW - 60.f, _settings.fovF, 60.f,
               110.f, "%.0f");
    cy2 += rowH;
    _settings.fov = (int)_settings.fovF;
    drawSlider(dl, "Leaf Density", lx, cy2, panW - 60.f, _settings.leafDensity,
               0.f, 5.f, "%.2f");
    cy2 += rowH;
    ...
    cy2 += rowH;
    drawSlider(dl, "Music Volume", lx, cy2, panW - 60.f, _settings.musicVolume,
               0.f, 1.f, "%.0f%%");
    cy2 += rowH;
    drawSlider(dl, "SFX Volume", lx, cy2, panW - 60.f, _settings.sfxVolume, 0.f,
               1.f, "%.0f%%");
  } else if (_settingsTab == 2) {
    drawSectionHeader(dl, font, "SERVER", lx, cy2, panW - 60.f);
    cy2 += 22.f;
    drawTextInput(dl, "Default Server IP", lx, cy2, 200.f, _settings.lastServer,
                  sizeof(_settings.lastServer));
    cy2 += rowH;
    drawInputInt(dl, "Default Port", lx, cy2, panW - 60.f, _settings.serverPort,
                 1024, 65535);
    cy2 += rowH;
    drawSectionHeader(dl, font, "AUTH", lx, cy2, panW - 60.f);
    cy2 += 22.f;
  ...
  if (menuButton(dl, "APPLY & SAVE", panX + panW - 180.f + 80.f, bbY, 160.f,
                 36.f, false, hApply))
    _settings.save();
...
// ── World Select
// ──────────────────────────────────────────────────────────────
GameState MainMenu::drawWorldSel(ImDrawList *dl, float cx, float cy, int sw,
                                 int sh, float dt) {
  ...
  static const char *SUBTXT[] = {"Last played: Today", "Last played: Yesterday",
                                 "New World"};
  float wy = panY + 62.f;
  for (int i = 0; i < 3; i++) {
    float rx = panX + 20.f, rw = panW - 40.f, rh = 54.f;
    bool hover = ImGui::IsMouseHoveringRect({rx, wy}, {rx + rw, wy + rh});
    dl->AddRectFilled({rx, wy}, {rx + rw, wy + rh},
                      hover ? COL_BTN_HOVER : COL_BTN_NORM, 3.f);
    dl->AddRect({rx, wy}, {rx + rw, wy + rh},
                hover ? colAlpha(COL_TEAL, 0.5f)
                      : colAlpha(COL_PANEL_EDGE, 0.4f),
                3.f, 0, 1.f);
  ...
  bool hBack = false;
  if (menuButton(dl, "BACK", cx, panY + panH - 52.f, 140.f, 36.f, false,
                 hBack)) {
    _panelSlide = 1.f;
    return GameState::MainMenu;
  }
...
// ── Connecting
// ────────────────────────────────────────────────────────────────
GameState MainMenu::drawConnecting(ImDrawList *dl, float cx, float cy, int sw,
                                   int sh, float dt) {
  _connectTimer += dt;
  ...
  if (!_connectFailed) {
    ...
    dots[d] = 0;
    char msg[128];
    snprintf(msg, sizeof(msg), "Connecting to %s:%d%s", pendingServerIP.c_str(),
             pendingServerPort, dots);
    ImVec2 msz = font->CalcTextSizeA(18.f, 999.f, 0.f, msg);
    dl->AddText(font, 18.f, {cx - msz.x * 0.5f, cy - 10.f}, COL_WHITE, msg);
    if (_connectTimer > 8.f)
      _connectFailed = true;
  } else {
    ...
    if (menuButton(dl, "RETRY", cx - 45.f, cy + 10.f, 80.f, 32.f, false,
                   hRetry)) {
      _connectTimer = 0.f;
      _connectFailed = false;
      return GameState::Connecting;
    }
    if (menuButton(dl, "BACK", cx + 45.f, cy + 10.f, 80.f, 32.f, false,
                   hBack)) {
      pendingServerIP = "";
      return GameState::Multiplayer;
    }
...
GameState MainMenu::draw(float dt, int screenW, int screenH,
  ...
  case GameState::Multiplayer:
    next = drawMultiplayer(dl, cx, cy, screenW, screenH, dt);
    break;
  case GameState::Account:
    next = drawAccount(dl, cx, cy, screenW, screenH, dt);
    break;
  case GameState::Connecting:
    next = drawConnecting(dl, cx, cy, screenW, screenH, dt);
    break;
  default:
    break;
  }
