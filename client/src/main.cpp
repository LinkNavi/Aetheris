  ...
  while (!window.shouldClose()) {
    ...
    while (enet_host_service(host.get(), &ev, 0) > 0) {
      ...
      if (uiOpen) {
        ...
        // After: if (lightAttack)
        if (lightAttack && player.isSpawned()) {
          // Raycast forward, check for tree within melee range
          glm::vec3 origin = camera.position;
          glm::vec3 dir = camera.forward();
          float range = 3.5f;
          glm::vec3 hitPos = origin + dir * range;

          // Check if any tree instance is near the hit point
          bool hitTree = false;
      ...
      int rdXZ =
          (int)std::clamp((int)mainMenu.settings().renderDistance, 1, 255);
