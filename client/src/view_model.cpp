...
static VkShaderModule vmMakeModule(VkDevice dev,
  ...
  ci.pCode = code.data();
  VkShaderModule m;
  vmCheck(vkCreateShaderModule(dev, &ci, nullptr, &m),
          "viewmodel shader module");
  return m;
}

static void uploadBuffer(VkDevice device, VmaAllocator allocator,
                         VkCommandPool pool, VkQueue queue, const void *data,
...
void ViewModelRenderer::init(VkDevice device, VmaAllocator /*allocator*/,
  ...
  vmCheck(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pCI, nullptr,
                                    &pipeline),
          "viewmodel pipeline");
...
int ViewModelRenderer::loadMesh(VkDevice device, VmaAllocator allocator,
  ...
  uploadBuffer(device, allocator, pool, queue, verts.data(),
               verts.size() * sizeof(GltfVertex),
               VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, gpu.vertBuf, gpu.vertAlloc);

  uploadBuffer(device, allocator, pool, queue, inds.data(),
               inds.size() * sizeof(uint32_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
...
void ViewModelRenderer::drawDebugUI() {
  if (!uiVisible)
    return;
  if (activeMesh < 0 || activeMesh >= (int)transforms.size())
    return;
  ...
  ImGui::Begin("Viewmodel Transform", &transformOpen,
  ...
  ImGui::Separator();
  ImGui::Text("Mesh Center: %.1f, %.1f, %.1f", t.meshCenter.x, t.meshCenter.y,
              t.meshCenter.z);
  ImGui::Separator();
  ...
  ImGui::TextDisabled("  Off(%.3f %.3f %.3f)  Rot(%.1f %.1f %.1f)", animOff.x,
                      animOff.y, animOff.z, animRot.x, animRot.y, animRot.z);

  ImGui::Separator();
  ImGui::TextWrapped("Copy to main.cpp:");

  char copyBuf[256];
  snprintf(copyBuf, sizeof(copyBuf),
           "t.offset={%.4ff,%.4ff,%.4ff};\nt.rotation={%.1ff,%.1ff,%.1ff};\nt."
           "scale={%.5ff,%.5ff,%.5ff};",
           t.offset.x, t.offset.y, t.offset.z, t.rotation.x, t.rotation.y,
...
void ViewModelRenderer::draw(VkCommandBuffer cmd, const glm::mat4 &proj) const {
  ...
  model = model * glm::eulerAngleXYZ(glm::radians(t.rotation.x + animRot.x),
