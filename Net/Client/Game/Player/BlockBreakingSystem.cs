// Net/Client/Game/Player/BlockBreakingSystem.cs - Handles breaking blocks and dropping items
using System;
...
namespace Aetheris
{
	...
	public void StartBreaking(Vector3i blockPos, int toolId = 0)
	{
	    ...
	    breakProgress = 0f;
	    breakTime = GetBreakTime(block.BlockType, toolId);
	}
	...
	private void BreakBlock(Vector3i pos, int toolId)
	{
	    ...
	    int dropId = GetDropForBlock(block.BlockType, toolId);
	    int dropCount = GetDropCount(block.BlockType, toolId);
	...
	private float GetBreakTime(BlockType type, int toolId)
	{
	    float baseTime = type switch
	    {
		BlockType.Dirt or BlockType.Sand or BlockType.Gravel => 0.5f,
		BlockType.Grass => 0.6f,
		BlockType.Wood => 1.5f,
		BlockType.Leaves => 0.2f,
		BlockType.Stone => 2.0f,
		BlockType.Snow => 0.3f,
		_ => 1.0f
	    };
	...
	private float GetToolMultiplier(BlockType type, int toolId)
	{
	    bool isPickaxeBlock = type == BlockType.Stone || type == BlockType.Gravel;
	    bool isShovelBlock = type == BlockType.Dirt || type == BlockType.Sand || type == BlockType.Grass || type == BlockType.Snow;
	    bool isAxeBlock = type == BlockType.Wood;
	...
	private int GetDropForBlock(BlockType type, int toolId)
	{
	    return type switch
	    {
		BlockType.Stone => 1,   // Stone item
		BlockType.Dirt => 2,    // Dirt item
		BlockType.Grass => 2,   // Drops dirt
		BlockType.Sand => 4,    // Sand item
		BlockType.Snow => 5,    // Snow item
		BlockType.Gravel => 6,  // Gravel item
		BlockType.Wood => 7,    // Wood item
		BlockType.Leaves => 0,  // No drop (could add leaf/sapling)
		_ => 0
	    };
	}
	
	private int GetDropCount(BlockType type, int toolId)
	{
	    return type == BlockType.Leaves ? 0 : 1;
	}
