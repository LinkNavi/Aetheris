...
using NetworkBlockType = Aetheris.BlockType;
...
namespace Aetheris
{
    public class BlockPlacementSystem
    {
        ...
        private void TryPlaceBlock()
        {
            ...
            // Store block type from ITEM (client-side rendering type)
        ...
        private void PlaceBlockAt(int x, int y, int z, ClientBlockType clientBlockType)
        {
            // Convert client block type to network block type
            NetworkBlockType networkBlockType = ConvertToNetworkBlockType(clientBlockType);

            // FIXED: PlacedBlocks.PlaceBlock expects Aetheris.BlockType (network type)
            bool placed = game.PlacedBlocks.PlaceBlock(x, y, z, networkBlockType);
            ...
            if (client != null)
            {
                _ = SendBlockPlacement(x, y, z, networkBlockType);
            }

            Console.WriteLine($"[BlockPlace] Placed {clientBlockType} block model at ({x}, {y}, {z})");
        }

        private async System.Threading.Tasks.Task SendBlockPlacement(int x, int y, int z, NetworkBlockType blockType)
        {
            await client.SendBlockPlaceAsync(x, y, z, blockType);
        }

        /// <summary>
        /// Convert client-side rendering BlockType to network protocol BlockType
        /// </summary>
        private NetworkBlockType ConvertToNetworkBlockType(ClientBlockType clientType)
        {
            // Map by name (assuming enums have same value names)
            // This is safe as long as both enums share the same value names
            return (NetworkBlockType)Enum.Parse(typeof(NetworkBlockType), clientType.ToString());
        }
